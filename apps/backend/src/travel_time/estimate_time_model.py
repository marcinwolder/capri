...
def get_travel_data():
	places_list, restaurants_list = get_attractions(
		None, City.get_const_krakow(), None, from_file=True
	)
	data = []
...
def train_model(car=True):
	with open(get_path('travel_data.json', 'time_estimation_models')) as file:
		data = json.load(file)
	...
	distances_train, distances_test, times_train, times_test = train_test_split(
