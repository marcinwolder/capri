...
class SplitForDays:
	...
	def split(self):
		...
		places_to_days = {
			place_id: days for place_id, days in places_to_days.items() if len(days) > 0
		}

		self.open_places = Places(
			[
				self.places.get_place_by_id(place_id)
				for place_id in places_to_days.keys()
			],
		...
		for i, (place_id, days) in enumerate(places_to_days.items()):
			...
			data.append(
				[
					...
					place.ratings.cumulative_rating * 10,
				]
			)
		...
		for i in range(len(self.open_places)):
			split_places[self.weekday_indices[cluster_assignment[i]]].append(
				self.open_places[i]
			)
		return [
			Places(split_places[i], city=self.open_places.city)
			for i in self.weekday_indices
		]
