...
from ..api_calls.wikipedia_description import get_summary_wikipedia_api
from ..ml_operations import kmeans_clustering
...
class Recommendation:
    ...
    def check_good_hours(self):
        for day in range(0, 7):
            for place in self.places.get_list():
                if (place.regularOpeningHours.periods[day].open_in_minutes <
    ...
    def get_recommendation(self):
        ...
        self.weather = get_weather_for_dates(self.places.city.lat, self.places.city.lng, self.dates[0], self.dates[1])
        print('weather', self.weather)

        places_positive_rating = Places([place for place in self.places.get_list()
                                         if place.ratings.cumulative_rating > 0])

        
        
        
        splitForDays = SplitForDays(from_date=self.dates[0], to_date=self.dates[1], places=places_positive_rating)
        ...
        for i, places in enumerate(clustered_places):
            route, transportations = Routing(places, depot=self.user_hotel, num_routes=1,
                            period_index=splitForDays.weekday_indices[i]).get_routes()
        ...
        for eating_time in eating_list:
            ...
            if id is not None:
                ...
                restaurants = get_restaurants_for_city(
                    db=None, lat=place.location.latitude, lng=place.location.longitude,
                    city=self.places.city, radius=500, included_types=params)

                if restaurants is not None and restaurants.count > 0:
                    calculate_cumulative_rating(places=restaurants,
    ...
    def _get_summary(self):
        ...
        self.summary = Llama.get_summary(city=self.places.city.name, trip=self.recommended_places)
        ...
        for i, places in enumerate(self.recommended_places):
            ...
            for j, place in enumerate(places.get_list()):
                try:
                    ...
                    transportation = None
                day_with_attributes.append(PlaceVisitor().place_to_itinerary(place, transportation))
            formatted_places.append(day_with_attributes)
        days = [{'places': places, 'weather': self.weather[i]}
                for i, places in enumerate(formatted_places)]
        itinerary = {"days": days, "summary": self.summary, 'dates': [d.isoformat() for d in self.dates],
                     'city_name': self.places.city.name, 'city_id': self.places.city.id}
